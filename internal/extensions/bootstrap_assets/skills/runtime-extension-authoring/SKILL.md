...
- If creating an extension host, use `luc.extension/v1` with `runtime.kind: exec`, `protocol_version: 1`, explicit `subscriptions`, and explicit `capabilities` for risky bidirectional features such as `client_actions`, `extensions.storage.session`, `extensions.storage.workspace`, or `tools.dynamic`. Extension hosts speak JSONL over stdin/stdout and can also own hosted tool handlers declared separately by `luc.tool/v2` manifests.
