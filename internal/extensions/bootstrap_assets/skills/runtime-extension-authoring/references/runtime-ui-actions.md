...
Runtime UI actions are host-owned. Tools, providers, and hooks with `client_actions` can request them through `client_action` events, and `luc.ui/v1` command manifests can trigger some of the same actions declaratively.
