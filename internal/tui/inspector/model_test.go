...
func TestInspectorViewAcrossTabs(t *testing.T) {
	model := New(workspace.Info{Root: "/tmp/work", ProjectID: "proj", HasGit: true}, "sess", "gpt-test")
	model.SetSize(40, 12)
	...
	if view := model.View(); !strings.Contains(view, "bash") {
		t.Fatalf("expected tool tab content, got %q", view)
	}

	model.NextTab()
	if view := model.View(); !strings.Contains(view, "started") {
		t.Fatalf("expected logs tab content, got %q", view)
	}

	model.NextTab()
	if view := model.View(); !strings.Contains(view, `"session_id": "sess"`) {
